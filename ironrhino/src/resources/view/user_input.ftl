...
<@s.form action="${getUrl(actionBaseUrl+'/save')}" method="post" cssClass="ajax form-horizontal">
	...
	<#else>
		<@s.textfield label="%{getText('username')}" name="user.username" cssClass="required checkavailable" dynamicAttributes={"data-checkurl":"${getUrl(actionBaseUrl+'/checkavailable')}"}/>
	...
	<@s.textfield label="%{getText('email')}" name="user.email" type="email" cssClass="email checkavailable" dynamicAttributes={"data-checkurl":"${getUrl(actionBaseUrl+'/checkavailable')}"}/>
