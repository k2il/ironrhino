...
if($('#_window_').length==0)
	$('<div id="_window_" class="flora" title=""><iframe style="width:600px;height:600px;"/></div>').appendTo(document.body);
...
if($('#_window_').attr('_dialoged_')){
	...
	$("#_window_").dialog({width:630,height:660,close:(reloadonclose?function(){ECSideX.reload();}:null)});	
},
...
if(url.indexOf('{parentId}')>0)
	...
	url+=(url.indexOf('?')>0?'&':'?')+'parentId='+parentId;
}
...
$.each(ECSideUtil.getEditedRows(ECSideX.id),function(){arr.push($(this).attr('recordkey'))});
$.each(arr,function(){
...
if(!name||name=='null'||$(this).attr('edited')!='true'&&$(this).hasClass('include_if_edited')) return;
...
arr=[];
$('input[type="checkbox"]',ECSideList[ECSideX.id].ECForm).each(function(){
	if(this.checked)
		arr.push('id='+$(this).parents('tr').attr('recordkey'));
});
...
updatePasswordCell:function(cellEditObj){
	...
	cellObj.innerHTML='********';
	cellObj.setAttribute("cellValue",ECSideUtil.trimString(cellEditObj.value));
...
Initialization.initECSideX=function(){if($(ECSideX.id)&&!ECSideX.instance){
