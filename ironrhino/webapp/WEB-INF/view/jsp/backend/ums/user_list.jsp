...
<div id="list" style="width: 100%"><ec:table tableId="ec"
	items="recordList" var="user" editable="true" action="user"
	title="users" nearPageNum="0" resizeColWidth="true" width="100%"
	styleClass="sortable">
	<ec:row recordKey="${user.id}">
		...
		<ec:column property="action" width="200px" viewsAllowed="html"
			...
			<button type="button" onclick="ECSideX.save('${user.id}')">保存</button>
			<button type="button" onclick="ECSideX.input('${user.id}')">编辑</button>
			<button type="button" onclick="ECSideX.del('${user.id}')">删除</button>
		</ec:column>
	</ec:row>
