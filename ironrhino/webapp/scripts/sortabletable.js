var SortableTable = {
	init : function(table, o) {
		...
		$(cells).each(function() {
			...
			if ($(this).hasClass(SortableTable.options.sortFirstAscendingClass)
					|| $(this)
	...
	sort : function(table, index, order) {
		...
		order = order ? order : ($(cell).hasClass(op.descendingClass) ? 1 : -1);
	...
	types : {
		number : function(a, b) {
			// This will grab the first thing that looks like a number from a
		...
		date : function(a, b) { // must be standard javascript date format
			...
			return SortableTable.compare(a ? new Date(a).valueOf() : 0, b
					? new Date(b).valueOf()
					: 0);
		},
		time : function(a, b) {
