...
UrlUtils = {
	...
	isSameOrigin : function(a, b) {
		...
		var arra = ad.split('.');
		var arrb = bd.split('.');
		return (arra[arra.length - 1] == arrb[arrb.length - 1] && arra[arra.length - 2] == arrb[arrb.length - 2]);
	},
...
Message = {
	...
	showActionMessage : function(messages, target, error) {
		...
		for ( var i = 0; i < messages.length; i++)
			html += Message.compose(messages[i],
					error ? 'action-error alert alert-error'
	...
	showFieldError : function(field, msg, msgKey) {
		...
		if (field && $(field).length) {
			...
			if (field.is(':visible')) {
				var prompt = $(
						'<div class="field-error removeonclick"><div class="field-error-content">'
...
function ajaxOptions(options) {
	...
	var entries = (options.replacement
			...
			|| ($(options.target).prop('tagName') == 'FORM' ? $(target).attr(
					'id') : null) || Ajax.defaultRepacement).split(',');
	var arr = [];
...
function _observe(container) {
	if (!container)
		container = document;
	$('.chart,form.ajax,.ajaxpanel', container).each(
			function(i) {
				if (!$(this).attr('id'))
					$(this).attr(
							'id',
...
Initialization.common = function() {
	$(document)
			...
			.ajaxSuccess(
					function(ev, xhr) {
						...
						if (url) {
							$('body')
									.html(
											'<div class="modal"><div class="modal-body"><div class="progress progress-striped active"><div class="bar" style="width: 50%;"></div></div></div></div>');
			...
			.on(
					'click',
					'.action-error strong.force-override',
					function(e) {
						var msgcontainer = $(e.target).closest(
								'.message-container');
						if (msgcontainer.length) {
							...
							$('tr', button.closest('form')).removeData(
									'version').removeAttr('data-version');
							button.click();
						}
...
if (HISTORY_ENABLED) {
	...
	Initialization.history = function() {
		...
		$.history.init(function(hash) {
			if ((!hash && !_historied_) || (hash && hash.indexOf('!') < 0))
				return;
			var url = document.location.href;
			if (url.indexOf('#') > 0)
...
Observation.common = function(container) {
	...
	var ele = ($(container).prop('tagName') == 'FORM' && $(container).hasClass(
			'focus')) ? container : $('.focus:eq(0)', container);
	...
	if (typeof $.fn.treeTable != 'undefined')
		$('.treeTable', container).each(
				function() {
					$(this)
							.treeTable(
									{
										initialState : $(this).hasClass(
												'expanded') ? 'expanded'
												: 'collapsed'
									});
				});
	...
	$('.tiped', container).each(
			function() {
				...
				t.bind(options.trigger == 'hover' ? 'mouseenter'
					...
					function() {
						...
						if (t.data('popurl')) {
							if (!options.content && t.data('popurl'))
								options.title = MessageBundle
										.get('ajax.loading');
							t
									.bind(
											options.trigger == 'hover' ? 'mouseenter'
													: options.trigger,
											function() {
							...
							if (t.data('quiet')) {
								$.ajax({
									...
									success : function(json) {
										document.getElementById(id).load(json);
									}
								});
							} else {
							...
							var options = {
								beforeSubmit : function() {
									...
									if (options.pushState === false
											|| form
													.parents('.ui-dialog,.tab-content').length)
										pushstate = false;
...
var Nav = {
	...
	indicate : function(a) {
		...
		if (nb.length)
			...
			nb = $('<ul id="nav-breadcrumb" class="breadcrumb"/>').prependTo(
					$('#content'));
		nb.append(
				'<li>' + dropdown.children('a').text()
						+ ' <span class="divider">/</span></li>').append(
				'<li class="active">' + a.text() + '</li>');
	}
}
