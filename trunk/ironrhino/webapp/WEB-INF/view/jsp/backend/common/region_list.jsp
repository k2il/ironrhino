...
<div id="list" style="width: 100%"><ec:table tableId="ec"
	items="recordList" var="region" editable="true" toolbarLocation="none"
	action="region" title="regions" nearPageNum="0" resizeColWidth="true"
	width="100%" styleClass="sortable">
	<ec:row recordKey="${region.id}">
		<ec:column width="22px" cell="checkbox" headerCell="checkbox"
			alias="deleteFlag" value="1" viewsAllowed="html" headerClass="nosort" />
		<ec:column property="name" cellName="region.name" cellEdit="input" />
		<ec:column property="displayOrder" cellName="region.displayOrder"
			cellEdit="input" />
		...
		<ec:column property="action" width="200px" viewsAllowed="html"
			...
			<button type="button" onclick="ECSideX.enter('${region.id}')">进入</button>
			<button type="button" onclick="ECSideX.save('${region.id}')">保存</button>
			<button type="button" onclick="ECSideX.del('${region.id}')">删除</button>
		</ec:column>
	</ec:row>
	<ec:extend>
