 ...
 * http://www.dillerdesign.com/experiment/DD_belatedPNG/ Version: 0.0.7a
...
/*
 * PLEASE READ: Absolutely everything in this script is SILLY. I know this. IE's
 * rendering of certain pixels doesn't make sense, so neither does this code!
 */
...
var DD_belatedPNG = {
	...
	vmlOffsets : function(el) {
		...
		makeVisible(el.vml.color.shape, (size.L + (el.isImg ? 0 : size.bLW)),
				(size.T + (el.isImg ? 0 : size.bTW)), (size.W - 1),
				(size.H - 1), 0);
		makeVisible(el.vml.image.shape, (size.L + size.bLW),
				(size.T + size.bTW), (size.W), (size.H), 1);
		...
		var figurePercentage = function(axis, position) {
			...
			var horz = (axis == 'X');
			bg[axis] = Math
					.ceil(fraction ? ((size[horz ? 'W' : 'H'] * bg[axis]) - (size[horz ? 'w'
							: 'h'] * bg[axis]))
							: parseInt(position));
			if (bg[axis] == 0) {
				bg[axis]++;
		...
		if (bgR != 'repeat') {
			...
			el.vml.image.shape.style.clip = 'rect(' + c.T + 'px '
					+ (c.R + fudge) + 'px ' + c.B + 'px ' + (c.L + fudge)
					+ 'px)';
		} else {
			el.vml.image.shape.style.clip = 'rect(' + dC.T + 'px ' + dC.R
					+ 'px ' + dC.B + 'px ' + dC.L + 'px)';
		}
	},
	...
	fixPng : function(el) {
		...
		el.vml.image.shape.fillcolor = 'none'; /*
												 * Don't show blank white
												 * shapeangle when waiting for
												 * image to load.
												 */
		el.vml.image.fill.type = 'tile'; /* Ze magic!! Makes image show up. */
		el.vml.color.fill.on = false; /*
										 * Actually going to apply vml element's
										 * style.backgroundColor, so hide the
										 * whiteness.
										 */
...
try {
	...
	DD_belatedPNG
	.fix('.pngfix,input.required,input.openid,.field_error,.action_error,.field_error .close,.action_error .close');
