	...
	function preview(input) {
		...
		if (!p.length) {
			...
			if (!(t.prop('readonly') || t.prop('disabled')))
				p.click(function() {
					$(this).hide().prev('.sqleditor:input').show().focus();
				});
		}
		p.width(t.width()).css('height', t.height() + 'px').html(
				$.sqleditor.highlight(t.val())).show();
	...
	$.fn.sqleditor = function() {
		$(this).each(function() {
			...
			if (t.is(':input')) {
				...
				t.addClass('preview').html($.sqleditor.highlight(t.text()));
			}
		});
		return this;
	};
