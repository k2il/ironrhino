...
<#else>
<a href="${resultPage.renderUrl(1)}" title="${action.getText('firstpage')}"<#list dynamicAttributes?keys as attr> ${attr}="${dynamicAttributes[attr]?html}"</#list>>&lt;&lt;</a>
<a href="${resultPage.renderUrl(resultPage.previousPage)}" title="${action.getText('previouspage')}"<#list dynamicAttributes?keys as attr> ${attr}="${dynamicAttributes[attr]?html}"</#list>>&lt;</a>
</#if>
...
<a href="${resultPage.renderUrl(index)}"<#list dynamicAttributes?keys as attr> ${attr}="${dynamicAttributes[attr]?html}"</#list>>${index}</a>
</#list>
...
<#else>
<a href="${resultPage.renderUrl(resultPage.nextPage)}" title="${action.getText('nextpage')}"<#list dynamicAttributes?keys as attr> ${attr}="${dynamicAttributes[attr]?html}"</#list>>&gt;</a>
<a href="${resultPage.renderUrl(resultPage.totalPage)}" title="${action.getText('lastpage')}"<#list dynamicAttributes?keys as attr> ${attr}="${dynamicAttributes[attr]?html}"</#list>>&gt;&gt;</a>
</#if>
</div>
</#if>
</#macro>