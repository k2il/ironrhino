...
<body>
<@s.form id="upload_form" action="${getUrl(actionBaseUrl)}" method="post" cssClass="ajax view form-inline" replacement="files">
	...
	<table id="files" class="checkboxgroup table table-striped middle" style="margin-top:50px;">
		...
		<tr>
			<td class="checkbox"><#if entry.value><input type="radio" name="id" value="${entry.key}"/></#if></td>
