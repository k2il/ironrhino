...
public class TimerExecutor {
	...
	private Map<Period, Map<String, Boolean>> map = new HashMap<Period, Map<String, Boolean>>();
		...
		for (String beanName : beanNames) {
			if (StringUtils.isAlphanumeric(beanName)
				...
				for (Method m : methods) {
					...
					Map<String, Boolean> temp = map.get(timer.period());
					if (temp == null) {
						temp = new HashMap<String, Boolean>();
						map.put(timer.period(), temp);
					}
					temp.put(expression, timer.global());
				}
			}
		}
	...
	public void execute(final String expression, final boolean global) {
		if (executorService != null) {
			executorService.execute(new Runnable() {
				...
				public void run() {
					try {
						applicationContextConsole.execute(expression, global);
					} catch (Exception e) {
	...
	public void executeOnDayStart() {
		if (!membership.isLeader(Period.DAY_START.getFullname()))
			return;
		Map<String, Boolean> temp = map.get(Period.DAY_START);
